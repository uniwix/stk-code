 ...
 *               is searched.
 */
unsigned int NeuronAI::getNextSector(unsigned int index)
{
...
#undef AI_DOES_NOT_MOVE_FOR_DEBUGGING
    ...
    const std::vector<double> inputs({
        ...
        (double)m_kart->getSteerPercent(),
        });
    const std::vector<double> outs = m_neuron_network.compute(inputs);
    const float steer(outs[0]);
	const float acc((outs[1] + 1.) / 2.);
 ...
 */
void NeuronAI::handleSteering(float dt)
{
 ...
 */
void NeuronAI::handleItemCollectionAndAvoidance(Vec3 *aim_point,
                                                  int last_node)
{
#ifdef AI_DEBUG
 ...
 */
bool NeuronAI::handleSelectedItem(Vec3 kart_aim_direction, Vec3 *aim_point)
{
 ...
 */
bool NeuronAI::steerToAvoid(const std::vector<const ItemState *> &items_to_avoid,
 ...
 */
void NeuronAI::evaluateItems(const ItemState *item, Vec3 kart_aim_direction,
                               std::vector<const ItemState *> *items_to_avoid,
                               std::vector<const ItemState *> *items_to_collect)
{
 ...
 */
void NeuronAI::handleItems(const float dt, const Vec3 *aim_point, int last_node, int item_skill)
{
    m_controls->setFire(false);
    if(m_kart->getKartAnimation() ||
 ...
 */
void NeuronAI::handleBubblegum(int item_skill,
 ...
 *  \param item_skill The skill with which to use the item
 */
void NeuronAI::handleCake(int item_skill)
{
 ...
 *  \param item_skill The skill with which to use the item
 */
void NeuronAI::handleBowling(int item_skill)
{
 ...
 *  \param items_to_avoid The list of close bad items
 */
void NeuronAI::handleSwitch(int item_skill,
                              const std::vector<const ItemState *> &items_to_collect,
                              const std::vector<const ItemState *> &items_to_avoid)
{
 ...
 */
void NeuronAI::handleAccelerationAndBraking(int ticks)
{
 ...
 */
void NeuronAI::handleBraking(float max_turn_speed, float min_speed)
{
...
void NeuronAI::handleRescue(const float dt)
{
 ...
 */
void NeuronAI::handleNitroAndZipper(float max_safe_speed)
{
 ...
 */
int NeuronAI::computeSkill(SkillType type)
{
    if (type == ITEM_SKILL)
    {
        int item_skill = 0;
...
//-----------------------------------------------------------------------------
void NeuronAI::checkCrashes(const Vec3& pos )
{
...
*/
void NeuronAI::findNonCrashingPointNew(Vec3 *result, int *last_node)
{
    *last_node = m_next_node_index[m_track_node];
...
*/
 void NeuronAI::findNonCrashingPoint(Vec3 *aim_position, int *last_node)
{
#ifdef AI_DEBUG_KART_HEADING
    const Vec3 eps(0,0.5f,0);
    m_curve[CURVE_KART]->clear();
    ...
    *last_node = m_next_node_index[m_track_node];
    ...
    for(unsigned int j=0; j<100; j++)
    {
        ...
        auto steps = (unsigned int)( len / m_kart_length );
        if( steps < 3 ) steps = 3;
 ...
 */
void NeuronAI::determineTrackDirection()
{
 ...
 */
bool NeuronAI::canSkid(float steer_fraction)
{
 ...
 *  \param dt Time step.
 */
void NeuronAI::setSteering(float angle, float dt)
{
